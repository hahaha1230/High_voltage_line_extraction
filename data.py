...
def trainGenerator(batch_size, train_path, image_folder, mask_folder, aug_dict, image_color_mode="rgb",
    ...
    # 图片生成器，负责生成一个批次的图片，以生成器的形式给模型训练，带两个（*）号则是表示字典（dict）
    image_datagen = ImageDataGenerator(**aug_dict)
    mask_datagen = ImageDataGenerator(**aug_dict)
    # 使用flow_from_directory从文件夹中获取各个类别的数据。
    ...
    #获取训练数据的大小
    train_sample_num= len(image_generator.filenames)
    #获取验证数据的大小
    val_sample_num=len(mask_generator.filepaths)
...
def testGenerator2(test_path, target_size=(1024, 1024), flag_multi_class=False, as_gray=False):
    test_samples = os.listdir(test_path)
    # num_image = len(test_samples)
    for name in test_samples:
        img = io.imread(os.path.join(test_path, name))
        img = img / 255
        img = trans.resize(img, target_size)
        # img = np.reshape(img,img.shape+(1,)) if (not flag_multi_class) else img
        img = np.reshape(img, (1,) + img.shape)
