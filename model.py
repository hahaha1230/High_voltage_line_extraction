...
from keras.layers import Conv2D, MaxPooling2D, Dropout, UpSampling2D, Concatenate, concatenate, \
    add  # ,AtrousConvolution2D
...
# Contain various FCNs model, including FCN-8s/2s, SegNet, Unet, VGGUnet, ResUnet and D-ResUnet;
...
def unet(pretrained_weights=None, input_size=(1024, 1024, 3)):
    inputs = Input(input_size)
...
def unet(pretrained_weights=None, input_size=(1024, 1024, 3), classNum=1, learning_rate=1e-5):
    inputs = Input(input_size)
    #  2D卷积层
...
def ceshiunet2(pretrained_weights=None, input_size=(1024, 1024, 3)):
    inputs = Input(input_size)
    model = Sequential(
        Conv2D(64, 3, activation='relu', padding='same', input_shape=(1024, 1024,)),
    ...
    model.summary()
    # 编译模型， 定义损失函数为均方误差函数，优化函数为随机梯度下降，可以尝试更改优化函数的参数从而加快计算速度
...
def D_resunet(input_size=(256, 256, 3)):  # 含有四个下采样残差单元的D_ResUnet
    ...
    model = Model(inputs=inputs, outputs=main_path)
    model.compile(optimizer=Adam(lr=2e-4), loss=final_loss, metrics=[IoU])
